...
is hosted on [his Github](https://github.com/colbywhite/colbywhite.dev).
...
individual blog post route make a fetch for the file, pick out the data they need, and pass the
raw React component to a helper function from `mdx-bundler` which converts into a regular
React component, and finally we display it in our page like good ol' JSX.
...
mega-file with everything in it. There would be a tradeoff because we couldn't cache that mega-file, but `mdx-bundler` is quite fast.
The overall performance didn't seem to take a hit.
